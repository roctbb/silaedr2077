...
def checkShielder(bot, message, user, all_users):
    if usersData[usersData[user["id"]]["playtennisConnection"]["id"]]["attackChoice"] in usersData[user["id"]]["shieldChoice"]:
        ...
        if usersData[user["id"]]["score"][1] + 1 == 5:
            ...
            usersData[usersData[user["id"]]["playtennisConnection"]
                      ["id"]]["shieldChoice"] = []
            usersData[usersData[user["id"]]["playtennisConnection"]
                      ["id"]]["playtennisConnection"] = None
    ...
    if not usersData[user["id"]]["wait"]:
        ...
        if usersData[user["id"]]["stage"] == 5:
            if message.text == "Выйти":
                ...
                bot.send_message(
                    user["id"], "Игра окончена", reply_markup=basemarkup)
                bot.send_message(usersData[user["id"]]["playtennisConnection"]["id"],
                                 "Противник вышел, игра окончена", reply_markup=basemarkup)
                usersData[usersData[user["id"]]["playtennisConnection"]
                          ["id"]]["playtennisConnection"] = None
                usersData[user["id"]]["playtennisConnection"] = None
            else:
                bot.send_message(
                    user["id"], "Ожидай оппонента или нажми 'Выйти'")
        # choosing
            ...
            if usersData[user["id"]]["turn"] == -1:
                ...
                if len(usersData[user["id"]]["shieldChoice"]) == 2:
                    if usersData[usersData[user["id"]]["playtennisConnection"]["id"]]["stage"] == 4:
                        bot.send_message(
                            user["id"], "Ты сделал выбор, ожидай противника...", reply_markup=exitmarkup)
        ...
        # cansel if u are tennis host + waiting for opponent to confirm -----------------------------
            ...
            if message.text == "Выйти":
                usersData[user["id"]]["stage"] = 0
                bot.send_message(
                    user["id"], "Вы вышли в главное меню", reply_markup=basemarkup)
                usersData[usersData[user["id"]]
                          ["playtennisConnection"]["id"]]["stage"] = 0
                bot.send_message(usersData[user["id"]]["playtennisConnection"]["id"],
                                 f"Ваш оппонент отозвал предложение", reply_markup=basemarkup)
                usersData[usersData[user["id"]]["playtennisConnection"]
                          ["id"]]["playtennisConnection"] = None
                usersData[user["id"]]["playtennisConnection"] = None
            else:
        ...
        # choice if someone wants to play with u -----------------------------
            ...
            if message.text == "Нет":
                bot.send_message(user["id"], "Окей, отмена",
                                 reply_markup=basemarkup)
                usersData[user["id"]]["stage"] = 0
                n = user["name"]
                bot.send_message(usersData[user["id"]]["playtennisConnection"]
                                 ["id"], f"{n} отказался :(", reply_markup=basemarkup)
                usersData[usersData[user["id"]]
                          ["playtennisConnection"]["id"]]["stage"] = 0
                usersData[usersData[user["id"]]["playtennisConnection"]
                          ["id"]]["playtennisConnection"] = None
                ...
                bot.send_message(user["id"], "Начинаем игру!",
                                 reply_markup=tennisgamemarkup)
                usersData[user["id"]]["stage"] = 4
                bot.send_message(usersData[user["id"]]["playtennisConnection"]["id"],
                                 "Оппонент согласен, начинаем игру!", reply_markup=tennisgamemarkup)
                usersData[usersData[user["id"]]
                          ["playtennisConnection"]["id"]]["stage"] = 4
                ...
                if usersData[user["id"]]["turn"] == 1:
                    ...
                    bot.send_message(
                        usersData[user["id"]]["playtennisConnection"]["id"], "Ты атакуешь, выбери куда будешь бить")
                usersData[usersData[user["id"]]["playtennisConnection"]
                          ["id"]]["turn"] = usersData[user["id"]]["turn"]*-1
        # opponent choice -----------------------------
            ...
            if message.text == "Отмена":
                ...
                data = []
                for i in all_users:
                    if usersData[i["id"]]["stage"] in [0, 1]:
                        data.append(i["name"])
                if message.text in data and message.text != user["name"]:
                    ...
                    for i in all_users:
                        if i["name"] == message.text:
                            ...
                            break
                    bot.send_message(
                        user["id"], "Ожидаем подтверждение от " + message.text + "...", reply_markup=exitmarkup)
                    n = user["name"]
                    usersData[user["id"]]["stage"] = 3
                    bot.send_message(usersData[user["id"]]["playtennisConnection"]["id"],
                                     f"Игрок {n} предлагает вам сыграть в пинг-понг", reply_markup=choicemarkup)
                    usersData[usersData[user["id"]]["playtennisConnection"]
                              ["id"]]["playtennisConnection"] = user
                    usersData[usersData[user["id"]]
        ...
        # base menu -----------------------------
            ...
            if message.text == "Наст. Теннис":
                if len(all_users) > 1:
                    ...
                    bot.send_message(
                        user["id"], "Ты можеш сыграть, выбери игрока", reply_markup=markupPlayers)
                else:
                    bot.send_message(
