...
outsidemarkup = create_keyboard(
    [["Болото", "Лес", "Спорт площадка"], ["Назад"]])

floor3markup = create_keyboard(
    [["Отмена", "Столовая", "Медпункт"], list(rooms[3])], rowsWidth=7)
...
def message(bot, message, user, all_users, location):
    if location["usersData"][user["id"]]["stage"] == 0:
        ...
        if message.text == "Подвал":
            ...
            bot.send_message(user["id"], "Выбери куда пойти",
                             reply_markup=outsidemarkup)
            location["usersData"][user["id"]]["stage"] = 1
        elif message.text == "3 этаж":
            bot.send_message(
                user["id"], "Выбери номер комнаты/столовку/медпункт", reply_markup=floor3markup)
            location["usersData"][user["id"]]["stage"] = 2
        elif message.text == "4 этаж":
            bot.send_message(
                user["id"], "Выбери номер комнаты", reply_markup=floor4markup)
        ...
        if message.text == "Назад":
            bot.send_message(
                user["id"], "Выбери куда пойти", reply_markup=basemarkup)
        ...
        elif message.text == "Медпункт":
            print(1)
            move_player(bot, user, "first_aid_station")
