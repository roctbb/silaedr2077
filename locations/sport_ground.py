...
basemarkup = helpers.create_keyboard([["Волейбол", "Футбол"], ['Выйти']])

playmarkup = helpers.create_keyboard(
    [["Серия пенальти(2 игрока)", "Футбол(4, 6 или 8 игроков, 2 команды)"]])


def enter(bot, user, all_users, location):
    if user["id"] not in location["players"]:
        bot.send_photo(user["id"], open("assets/sport_ground/sport_ground.jpg", "rb"),
                       caption="Приветствуем на спорт площадке!\n Сейчас здесь находится"+str({len(all_users)})+"людей.", reply_markup=basemarkup)
    else:
        bot.send_photo(user["id"], open("assets/sport_ground/sport_ground.jpg", "rb"),
                       caption="С возвращением\nИгроков: "+str({len(all_users)})+", удачной игры!", reply_markup=basemarkup)
...
def message(bot, message, user, all_users, location):
    ...
    if message.text == "Поиграть в футбол":
        bot.send_message(user["id"], "Выберите режим игры",
                         reply_markup=playmarkup)
